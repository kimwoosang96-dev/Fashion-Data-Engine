...
> 작성일: 2026-03-01 | 최종 업데이트: 2026-03-01 (Phase 21 기준)
...
│   └── lib/                  ← API 호출 함수, 타입 정의
│
├── scripts/                  ← 실행 스크립트 31개 (크롤, 감사, 시드, 정제 등)
├── alembic/                  ← DB 마이그레이션 (14개 버전)
...
**현황**: 0개 (build_product_catalog.py 미실행). 실행 필요.
...
| `build_product_catalog.py` | ProductCatalog 빌드 (normalized_key 집계) |
...
### 로컬 SQLite `data/fashion.db` (2026-03-01 기준)
...
| **제품** | |
| 전체 제품 | 80,096개 |
| 활성 제품 (is_active=True) | 80,096개 (100%) |
...
| Route 배송보험 제품 오인덱싱 | 높음 | Shopify `/products.json`에 Route 앱 제품이 패션 제품처럼 노출 | **T-054 과업지시 완료 / Codex 구현 예정** |
| brand-store 49개 미크롤 | 중간 | 크롤 미실행 또는 비표준 플랫폼 | 진행 중 |
| edit-shop 크롤 25개 미완 | 높음 | Cafe24 커스텀 전략 미등록 | 진행 중 |
| 환율 자동 업데이트 없음 | 중간 | Railway Worker 미설정 | 미해결 |
| ProductCatalog 비어있음 | 중간 | build_product_catalog.py 미실행 | 미해결 |
...
| Task ID | 제목 | 상태 |
|---------|------|------|
| T-20260301-054 | PRODUCT_DENYLIST_01: Route 배송보험 등 비패션 제품 필터링 | 과업지시 완료, Codex 구현 예정 |
| T-20260301-055 | CHANNEL_TRAFFIC_LIGHT_01: 채널 크롤 건강도 트래픽 라이트 | 과업지시 완료, Codex 구현 예정 |

GPT Pro 채널 리서치: `docs/channel_research_gpt.md` 참조
...
1. **T-054 비패션 제품 필터 구현** → Route 배송보험 등 노이즈 제거 (Codex 구현 예정)
2. **T-055 채널 트래픽 라이트** → 크롤 실패 채널 즉각 파악 (Codex 구현 예정)
3. **Railway Worker 설정** → 스케줄러가 자동으로 돌아야 크롤/환율/알림 작동
4. **채널 리서치 완료** → 78개 0-product 채널 플랫폼/상태 파악 (GPT Pro 의뢰 중)
5. **ProductCatalog 빌드** → `build_product_catalog.py` 실행 필요
6. **사용자 계정 시스템** → 구매이력/관심목록의 실용성을 위해 필요

---

*최종 업데이트: 2026-03-01 (Phase 21 설정 기준)*
