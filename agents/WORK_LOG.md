...
- 2026-03-08 23:15:00 | agent:codex-dev | task:T-20260308-145 | action:complete | message:CHANNEL_HEALTH_DASHBOARD_01 완료: `channel_crawl_stats` 최근 5회 yield를 기준으로 `/admin/channel-health` API와 Next 페이지를 구현했다. healthy/degraded/dead 상태 계산, 최근 성공 시각/parse method 표시, `POST /admin/channels/{id}/reactivate` probe 재활성화 버튼까지 연결.
- 2026-03-08 23:15:30 | agent:codex-dev | task:T-20260308-146 | action:complete | message:API_PERFORMANCE_MONITOR_01 완료: `monitoring.py`로 응답시간/슬로쿼리 메모리 집계 추가, FastAPI 미들웨어와 SQLAlchemy 500ms 이상 쿼리 기록을 연결했다. `/admin/performance` API와 시간당 `performance_alert_hourly` 스케줄 잡을 추가했고 `scheduler.py --dry-run`에서 등록을 확인했다.
- 2026-03-08 23:16:00 | agent:codex-dev | task:T-20260308-147 | action:complete | message:DATA_FRESHNESS_01 완료: `channels.last_crawled_at` 컬럼 및 Alembic `5d6e7f8a9b0c`를 추가하고 `crawl_products.py`에서 성공 채널 갱신을 반영했다. `/channels`, `/api/v2/availability`, 제품 상세에서 freshness 시간 표기를 추가했다.
- 2026-03-08 23:16:30 | agent:codex-dev | task:T-20260308-142 | action:log | message:LLM_COST_TRACKING_01 코드 완료: `channel_crawl_stats`에 토큰/비용 컬럼을 추가하고 GPT fallback 결과의 prompt/completion/provider/cost 기록과 `/admin/llm-costs` 집계 API를 구현했다. 실제 GPT 크롤 한 번으로 운영 검증이 남아 있다.
- 2026-03-08 23:17:00 | agent:codex-dev | task:T-20260308-143 | action:log | message:DB_BACKUP_AUTOMATION_01 코드 완료: `scripts/backup_db.py`, S3/R2 환경변수, `backup_db_weekly_sun_0200` 스케줄을 추가했다. S3 자격증명과 복원 테스트가 없어 운영 검증은 보류 상태다.
- 2026-03-08 23:17:30 | agent:codex-dev | task:T-20260308-144 | action:log | message:SENTRY_MONITORING_01 코드 완료: backend `sentry-sdk[fastapi]`, frontend `@sentry/nextjs`, Next instrumentation/global-error, crawl_products 예외 capture를 추가했다. `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN` 환경설정 후 의도적 에러 캡처 확인이 남아 있다.
